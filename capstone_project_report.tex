...
python create_and_analyze_pascal_tf_record.py --data_dir=./data/voc_format --label_map_path=./pascal_label_map.pbtxt --output_path=./data/
\end{lstlisting}
%
...
Further, we create a dictionary that maps our logo type to a unique id in form of a \path{pascal_label_map.pbtxt} containing 821 entries like the following:
\begin{lstlisting}[language=Python]
item {
  id: 263
  name: 'starbucks-text'
...
\end{lstlisting}
This results in "Test accuracy: 22.9852\%". In the next section, we describe how we fine-tune our first model. 
\subsubsection*{A Custom Model on top of the VGG16 model \cite{DBLP:journals/corr/SimonyanZ14a} in Keras}
Now we continue to build a custom Keras CNN model on top of the well-known VGG16 architecture: 
\begin{lstlisting}[language=Python]
...
\end{lstlisting}
We monitor statistics with \path{tensorboard --logdir=PATH_TO/LogosInTheWild-v2/models/model/}.
...
\textbf{\textcolor{red}{Unfortunately the Faster R-CNN does not detect any logos, please help me with Feedback!}}
...
%Example:
%
%ssh -i "BasicE2.pem" ec2-user@ec2-18-195-165-55.eu-central-1.compute.amazonaws.com  -L 8889:127.0.0.1:8889

%scp -r -i "BasicE2.pem" /data/Machine_Learning/Machine_Learning_Engineer/capstone_project/LogosInTheWild-v2/data/ ec2-user@ec2-3-120-244-179.eu-central-1.compute.amazonaws.com:capstone_project/LogosInTheWild-v2/data/ 
