...
Next we execute the Python script create\_clean\_dataset.py from the \path{LogosInTheWild-v2/scripts} directory that adjusts the brand names in the XML files and removes XML files without corresponding JPEG image.
This outputs that 9,428 images and 821 brands were processed, while 1,330 JPEG files were unavailable.

For our first logo classification task with a standard CNN architecture in Keras, we can load the image files with the \verbatim{load_files} function from skearn.datasets since the Logos in the Wild dataset is organized in directories with corresponding brand names. Of course, some images like 

Finally, for Tensorflow's Object Detection API we need to convert our dataset with annotations in Pascal-VOC style to Tensorflow's TFRecord file format. For this, we copy their modified Python script \path{create_pascal_tf_record.py} from the Capstone project folder to the LogosInTheWild-v2 directory and run the following:
%
...
After completion we see that we converted 7,542 images with annotations into the 10 training TFRecord files \path{pascal_train.record-0000i-of-00010}, and 1,886 images with annotations into the 10 validation TFrecord files \path{pascal_val.record-0000i-of-00010} for $i=0,\ldots,9$.

Among other parameters for our Faster R-CNN, we choose in our configuration file \path{faster_rcnn_inception_logos.config} by the Capstone Project as data\_augmentation\_options random\_horizontal\_flip, random\_vertical\_flip and random\_rotation90.
...
export PYTHONPATH=$PYTHONPATH:`pwd`:`pwd`/slim
PIPELINE_CONFIG_PATH=/data/Machine_Learning/Machine_Learning_Engineer/capstone_project/logo-bucket-europe-west1/LogosInTheWild-v2/models/model/faster_rcnn_inception_logos-locally-on-ubuntu.config
MODEL_DIR=/data/Machine_Learning/Machine_Learning_Engineer/capstone_project/logo-bucket-europe-west1/LogosInTheWild-v2/models/model/
NUM_TRAIN_STEPS=20000
SAMPLE_1_OF_N_EVAL_EXAMPLES=1
TRAIN_DIR=/data/Machine_Learning/Machine_Learning_Engineer/capstone_project/logo-bucket-europe-west1/LogosInTheWild-v2/models/model/train
EVAL_DIR=/data/Machine_Learning/Machine_Learning_Engineer/capstone_project/logo-bucket-europe-west1/LogosInTheWild-v2/models/model/eval
python object_detection/model_main.py --pipeline_config_path=${PIPELINE_CONFIG_PATH} --model_dir=${MODEL_DIR} --num_train_steps=${NUM_TRAIN_STEPS} --sample_1_of_n_eval_examples=$SAMPLE_1_OF_N_EVAL_EXAMPLES --alsologtostderr
...
%Hi @danyfang! You meant --model_dir, right? I specify that for tensorboard but still it shows no new results from model_main.py, only the old logs from /legacy/train.py and /legacy/eval.py. I corrected now the num_classes to 